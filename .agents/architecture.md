...
2. Activates `flutter_distributor` from the `chen08209/flutter_distributor` fork, pinned to a `v<version>-flclash.<n>` tag
   rather than the moving `FlClash` branch; cut a new tag there and bump the `--git-ref` when the fork changes.
...
- Windows and Linux build `FlClashCore`, the Rust `FlClashHelperService` privileged helper, and a
...
Go core and, on Windows and Linux, a separate Rust helper. Per-target records live under `.dart_tool/setup_build_cache/v1/`:
...
- `setup`: build-time harness for Go core artifacts and the Rust helper; no runtime Dart API.
