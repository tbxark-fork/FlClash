...
It writes the tray PNGs with Flutter `2.0x/`–`4.0x/` resolution variants, multi-size tray `.ico`
files, and `windows/runner/resources/app_icon.ico` from `assets/images/icon.png`. The `pubspec.yaml`
entries for these files carry `platforms:` so each build only bundles the format its tray loads;
a new status icon needs its own PNG and ICO entries there.
